...
class AccountService {
  /**
   * GENERATE ACCOUNT CODE (CORE LOGIC)
   *
  ...
  static async generateAccountCode(parentId, tenantId) {
    ...
    if (!parentId) {
      ...
      // Predefined prefixes: 10, 40, 50, 60, 70
      ...
      if (children.length === 0) {
        ...
        if (parent.level === 1) {
          // Child of level 1: add 01 to next segment
          code = `${parentPrefix}010000`;
        } else if (parent.level === 2) {
          // Child of level 2: add 01 to next segment
          code = `${parentPrefix}0100`;
        } else if (parent.level === 3) {
          // Child of level 3: add 01 to next segment
          code = `${parentPrefix}01`;
        }
      } else {
        // Subsequent children: increment last segment
        const lastChild = children[0].code;
        const lastSegment = parseInt(lastChild.substring(2 * parent.level, 2 * parent.level + 2));
  ...
  static async createAccount(data, user, tenantId) {
    ...
    data.isSystem = false; // User-created accounts can be disabled
  ...
  static async getAccountTree(tenantId, type = null) {
    const query = { tenantId, isActive: true, level: 1 };
  ...
  static async buildAccountTree(accountId, tenantId) {
    ...
    const children = await Account.find(
      { tenantId, parentId: accountId, isActive: true },
      {},
      { sort: { code: 1 } }
    ).lean();
  ...
  static async getAccountWithHierarchy(accountId, tenantId) {
    ...
    while (current.parentId) {
      const parent = await Account.findById(current.parentId).lean();
      if (!parent) break;
  ...
  static async disableAccount(accountId, tenantId, user, reason = '') {
    ...
    // Prevent disabling system accounts
    if (account.isSystem) {
      throw new Error('System accounts cannot be disabled');
    }

    // Check if account has active postings
    const hasPostings = await this.checkIfAccountHasPostings(accountId);
    if (hasPostings) {
      throw new Error('Cannot disable account with existing postings. Create a new account for future entries.');
    }

    // Prevent disabling parent if children exist
   ...
   */
  static async checkIfAccountHasPostings(accountId) {
    // This will be implemented once Journal module is ready
    // For now, return false (no postings exist)
    // TODO: Check Journal collection for entries with this accountId
    return false;
  }
   ...
   * (Used in reports)
   */
  static async getAccountsByTypeWithData(tenantId, type) {
    ...
    // TODO: Add balance information from Journal once available
    return accounts.map(acc => ({
      ...acc,
      balance: acc.openingBalance || 0, // Will be calculated from postings
      debitTotal: 0,
      creditTotal: 0
    }));
  }
